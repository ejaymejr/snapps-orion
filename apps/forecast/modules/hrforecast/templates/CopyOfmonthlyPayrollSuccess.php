...
<!-- 	LEVY CHART -->
	...
	<?php 	
	//prepare graph data
	$permonthTotalTrend = array();
	foreach($months as $month):
		$permonthTotalTrend[] = array(
		  'month' => DateUtils::DUFormat('M-Y', HrLib::GetDateByPeriod( $month) )
		, 'total' => $totalPerMonth[$month]['ot_normal'] + $totalPerMonth[$month]['ot_offday'] + $totalPerMonth[$month]['ot_holiday']
		, 'ot_normal' => $totalPerMonth[$month]['ot_normal']
		, 'ot_offday' => $totalPerMonth[$month]['ot_offday']
		, 'ot_holiday' => $totalPerMonth[$month]['ot_holiday']
		); 
	endforeach;
	$perMonthTotalTrendJson = json_encode($permonthTotalTrend);
	
// 	                   echo '<pre>';
// 			            print_r($permonthTotalTrend);
// 			            echo '</pre>';
// 			            exit();
	?>
	...
	<div class="panel " >
		...
		<script type="text/javascript">
            ...
            var chartData2 = <?php echo $perMonthTotalTrendJson; ?>;
