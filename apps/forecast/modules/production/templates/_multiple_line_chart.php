...
$chartTitle = isset($chartTitle) ? 'No Title' : $title;
...
//$plotDatas = array('income'=>array('values'=>array(), 'total'=>0));
//foreach ($plots as $pl) {
//    $plotDatas["$pl"] = array();
...
//    foreach ($record['reject_type'] as $rejectType => $tmp) {
...
//var_dump($plotDatas);
//exit();
//foreach($plotDatas['Income'] as $ke => $ve):
//	$plotDatas['margin'][] = $plotDatas['Income'][$ke] - $plotDatas['Expense'][$ke];
//endforeach;
...
foreach ($plotDatas as $rejectType => $tmp) {   
	$color = "color='#" . $colors . "'"; 
    $chartData .= '<graph ' . $color . ' balloon_text=\'{description}    {value}\' title=\'' . $rejectType . '\' >';       
    for ($i = 0; $i < sizeof($tickLabels); $i++) {
        $chartData .= '<value xid=\'' . $i . '\' description=\'' . $ballonLabels[$i] . '\' >' . $tmp[$i] . '</value>';
    }
    $chartData .= '</graph>';
}
$chartData .= '</graphs>';
...
$width = 250 + ( 50 * sizeof($tickLabels));
...
<script type="text/javascript">
    // <![CDATA[        
    var so = new SWFObject("<?php echo sfConfig::get('app_amline_public_swf'); ?>", "amline", "<?php echo $width ?>", "300", "8", "#FFFFFF");
...
<tr>
    ...
    <?php foreach ($tmp as $k => $val) : ?>
    <td class="alignRight"><?php echo HTMLText::FormatCellNumber($val); ?></td>
    <?php $tot += $val; endforeach; ?> 
    <td class="section-header alignRight"><?php echo HTMLText::FormatCellNumber($tot); ?></td>    
</tr>
<?php endforeach; ?> 
</table>
