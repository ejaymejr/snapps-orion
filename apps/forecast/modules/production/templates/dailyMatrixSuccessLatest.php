...
<?php
	...
	{ 
		...
		$twdith = '600';
		$dgroup = array();
		 $otherGrp = HrEmployeePeer::OtherGroup($co);   
         $dept = array();
         ...
         $dept = $sf_params->get('mcsother')? array_merge($dept, $otherGrp) : $dept;

        	$empWorking = TkDtrsummaryPeer::GetTotalManWorkingbyDateRange($sdate, $edate, $co, $dept);
        	$empList = $empWorking['list'];
        	//var_dump($empWorking);
        	//exit();
        	...
        	foreach($empList as $ke=>$empNo)
        	{
        	    ...
        	    for($x=0; $x< DateUtils::DateDiff('d', $sdate, $edate) + 1; $x++)
        	    {
        	        ...
        	        foreach( $empData as $rec)
        	        {
        	            ...
        	            {
        	            	//modify here kebot
        	            	//calculation is based on multiplier OT Hrs is x 1.5hrs
        	               //$nohrs = ($rec->getAcDura() > 0? $rec->getAcDura() : 0);
        	              
        	               $acdura  = $rec->getAcDura();
        	               $xplier = $rec->getMultiplier();
        	               $normal = $rec->getNormal();
        	               if ($acdura >= $normal):
        	               		$nohrs = $normal + ( $rec->getOvertimes() * $xplier);
        	               		//echo 'nohrs: ' . $nohrs .' OT: ' . $rec->getOvertimes();
        	               else:
        	               		$nohrs = $acdura;
        	               endif;
        	               //echo ' | Actual: '. $acdura . ' | Normal: ' . $normal .' | ';
        	               //echo $rec->getName() .' : '. $nohrs.'<br>';
        	               //$nohrs = 100;
        	               $dhrs = $dhrs + $nohrs;        	               
        	            }
        	        }
...
// 					//echo $cdate .': '.($dtTotal[$cdate] ) .' Income: '. $empInc . ' | xdt: '. $xdt. '<br>';
//   	                $incproof = $incproof .  '
...
//        	
//        	';
//        	$gTInc = 0; //grand total income
//        	for($x=0; $x< DateUtils::DateDiff('d', $sdate, $edate) + 1; $x++)
//        	{
//        		$cdate = DateUtils::AddDate($sdate, $x);
//        		$gTInc = $gTInc + $dtTotal[$cdate];
//        		$incproof = $incproof .  '
//                	<td width="30" class="alignCenter FORMcell-Center tk-pink"  nowrap>'.$dtTotal[$cdate].'</td>
//                ';
//        	}
//        	$incproof= $incproof . '<td width="30" class="alignCenter FORMcell-Center tk-pink"  nowrap>'.$gTInc.'</td>
//        	</tr>
//			</table>
//			
//			';   
