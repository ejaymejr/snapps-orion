...
foreach($plotDatas as $kData=>$vData)
{
	...
	for ($i = 0; $i < sizeof($tickLabels); $i++) {
	    $chartData .= '<value xid=\'dur' . $i . '\' description=\'' . $tickLabels[$i] . '\'>' . $plotDatas[$kData][$i] . '</value>';
	}
	$chartData .= '</graph>';
	$pos++;
}	
...
$chartWidth = 200 + (sizeof($tickLabels) * 60);
?>
...
<script type="text/javascript">
    // <![CDATA[        
    var so = new SWFObject("<?php echo sfConfig::get('app_amcolumn_public_swf'); ?>", "amcolumn", "<?php echo $chartWidth; ?>", "400", "8", "#FFFFFF");
