...
class individualLeaveAddAction extends SnappsAction
{
    ...
    public function execute()
    {
        if ($this->getRequest()->getMethod() == sfRequest::POST) 
        {
            ...
            $this->record->save();
            HrLib::LogThis($this->_U(), 'UPDATE LEAVE', '', $this->getModuleName().'/'.$this->getActionName(), $emp->getName()  );
            ...
            $this->redirect('employee/generalInformation?id=' . $emp->getId().'&active=leave' );
        }
    }
    
    public function validateindividualLeaveAdd()
    {
        $this->preExecute();
        if ($this->getRequest()->getMethod() != sfRequest::POST)
        {
