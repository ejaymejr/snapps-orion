...
$STACKED = (isset($stacked)? $stacked : 'stacked');
// get settings    
$settingFile = 'amcolumn_settings_clustered.xml';
$strs = file(   
...
foreach ($strs as $str) {
    ...
    if (strpos($str, '<!--') === false) {
        ...
        if ($STACKED == '100')
        {
			$str = str_replace('<type>stacked</type>', '<type>100% stacked</type>', $str);
        }        
...
$graphTypes =array(0 => 'Expense', 1 => 'Income');
foreach ($graphTypes as $type => $name) 
{
    $color = "color='#" . $colors[$pos] . "'";
	$chartData .= '<graph ' . $color . ' title=\'' . $name . '\'>';  

	$i = 0;
	foreach($plotDatas as $kData=>$vData)
	{       
		$chartData .= '<value xid=\'dur' . $i . '\' description=\'' . $ballonLabels[$i] . '\'>' . $vData[$type] . '</value>';
		$i++;
	}
	$chartData .= '</graph>';
	$pos++;
...
$width = 250 + ( 100 * sizeof($tickLabels));
?>
...
<script type="text/javascript">
    // <![CDATA[        
    var so = new SWFObject("<?php echo sfConfig::get('app_amcolumn_public_swf'); ?>", "amcolumn", "<?php echo $width ?>", "300", "8", "#FFFFFF");
