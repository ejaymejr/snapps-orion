...
$editLink = 'Edit';
$deleteLink = 'Delete';
$editLink = link_to('Edit', 'expenses/waterConsumptionEdit?id=' . $record->getId());
  
$deleteLink = link_to('Delete', 'expenses/waterConsumptionDelete?id=' . $record->getId(),
                    array('confirm' => 'Record [ '.$SN.': - '.$record->getDate() .' / '. $record->getAmpm() .' ]  Sure to delete this Water record?')); 


$checkBoxID = 'gridCheckBox_item_' . $record->getId();

$pconvert = 0;
$am = WaterUsagePeer::GetOptimizedDatabyDateAmPm($record->getDate(), 'AM', array( 'reading', 'consumption', 'total_cost', 'conversion_factor') );
...
$pm = WaterUsagePeer::GetOptimizedDatabyDateAmPm($record->getDate(), 'PM', array( 'reading', 'consumption', 'total_cost', 'conversion_factor') );
