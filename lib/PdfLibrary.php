...
class PdfLibrary extends FPDI_Protection
{
    ...
    public function printNumber($x, $y, $mess, $font = null, $fontsize = null)
    {
        $this->setMyFont('');
        if ($font)
        {
...
//     	exit();
    ...
    public function printNanocleanHeader($y=13, $withline = FALSE)
    {
    	//     	var_dump($this->orientation);
    	//     	exit();
    	if ( $this->orientation == 'L' ) :
    	...
    	$this->setMyFont('Arial', 'B', 18);
    	$this->pdf->SetXY($y, 2);
    	$this->pdf->Cell(1, 15, 'NANOCLEAN',0,1,'R');
    	$this->setMyFont('Arial', '', 10);
    	$this->pdf->SetXY( $y, 14);
    	$this->pdf->Cell(1, 1, '35 Senoko Way Woodlands East,',0,1,'R');
    	$this->pdf->SetXY( $y, 18);
...
//     	endif;
    	...
    	if ( ($user) == 'emmanuel' || ($user) == 'hr_super' || ($user) == 'florence'):
