...
class PhpUploader
{
	...
	/// chould be also set to web.config appSettings : CuteWebUI.AjaxUploader.FlashUploadPage (CuteEditor.AjaxUploader.FlashUploadPage for CuteEditor)	
	...
	{
//	    upload_resource: 
//	        path: /opt/httpd/htdocs_i/symfony/snapps-reject/apps/manage/lib/uploader/resources
//	        include_uploader_file: /opt/httpd/htdocs_i/symfony/snapps-reject/apps/manage/lib/uploader/phpuploader/include_phpuploader.php
//	        resource_handler: /opt/httpd/htdocs_i/symfony/snapps-reject/apps/manage/lib/uploader/phpuploader/ajaxuploaderresource.php		
		$this->Name="AjaxUploaderFiles";
		
		//$cd=dirname($this->GetWebPath(__FILE__));
//		$cd = sfConfig::get('app_upload_resource_path');
//		if (!$cd):
//			$cd = '/opt/httpd/htdocs_i/symfony/snapps-reject/web/uploader';
//		endif;		
		$cd = '../../uploader';
		$rd =  "$cd/resources/";
		$rh = "$cd/phpuploader/ajaxuploaderresource.php";
		$UplUrl = "$cd/phpuploader/ajaxuploaderhandler.php";

		
		$this->ResourceDirectory=$rd;
		$this->ResourceHandler=$rh;
		$this->UploadUrl=$UplUrl;
		
//		var_dump($this->UploadUrl);
//		exit();
...
//		var_dump("<script type='text/javascript' src='".$resourcehandler."?type=script'></script>");
//		exit();
		$code.="<img id='" . $this->Name . "_Loader' UniqueID='" . $this->Name . "' ";
		$code.=" Namespace='CuteWebUI' UploadModuleNotInstall='1' ServerLang='PHP' src='$resourcehandler?type=file&amp;file=continuous.gif'";
		...
		//TODO:Opera!!
		$code.=" onload='this.style.display=&quot;none&quot; ; CuteWebUI_AjaxUploader_Initialize(this.id);' onerror='this.onload()' ContextValue='" .session_id(). "' />";
		return $code;
	}
...
//		exit();
	...
	function _HandleAddonUpload()
	{
		...
		$addonpartial=@$_POST["CuteWebUI.AjaxUploader.Partial"];
		if(!$addonpartial)$addonpartial=@$_POST["CuteWebUI_AjaxUploader_Partial"];
	...
	function WriteValidationOK()
	{
		...
		echo("if(window.parent.CurrentUpload)window.parent.CurrentUpload.UploadOK('" . $uploadid . "','" . $this->_fileguid . "')");
		echo("</script>");
