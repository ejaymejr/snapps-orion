...
/**
 * Subclass for performing query and update operations on the 'power_usage' table.
 *
 * 
 *
 * @package lib.model.maintenance
 */ 
class PowerUsagePeer extends BasePowerUsagePeer
{
	...
	public static function GetPager($cd)
	{
		...
		$c = clone($cd);
		$pager = new sfPropelPager("PowerUsage", $rowsPerPage);
	...
	public static function getPreviousReading($dt)
	{
		...
		//this happen because on 2009-11-13 they change the meter...
	}
	...
	public static function GetPowerConsumptionbyDateRange($sdt, $edt, $ampm)
	{
		$c = new Criteria();
		$c->add(self::DATE,  'DATE(' . self::DATE . ') >= \'' . $sdt . '\' AND DATE(' . self::DATE . ') <= \'' . $edt . '\'', self::CUSTOM);
		$c->add(self::AMPM, $ampm);
	...
	public static function PostDailyData($sdate, $edate, $user)
	{
		...
		//        var_dump($usageData);
		//        exit();
		$component = 'ELECTRICAL UTILITIES < Rdng * 15 * $ 0.2 / KiloWatt Hour >'  ;
		while($usageData->next())
		{
			$gtota = $usageData->get('TOTAL_COST');
			$record = FinanceSummaryPeer::GetSourceCompanyAndDate($component, $tempCompany, $usageData->get('DATE'));
			if (! ($record) )
			{
	...
	public static function GetPowerReadingForTheDay($cdt)
	{
		$c = new Criteria();
		$c->add(self::DATE,  $cdt);
	...
	public static function GetPowerConsumptionForTheDay($cdt)
	{
		$c = new Criteria();
		$c->add(self::DATE,  $cdt);
		$c->add(self::AMPM,  'DL');
	...
	public static function DeletePowerConsumptionbyDateRange($sdt, $edt)
	{
		//     	$c = new Criteria();
		//     	$c->add(self::DATE,  'DATE(' . self::DATE . ') >= \'' . $sdt . '\' AND DATE(' . self::DATE . ') <= \'' . $edt . '\'', self::CUSTOM);
		//     	$c->add(self::AMPM, 'DL');
