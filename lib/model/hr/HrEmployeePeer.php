...
class HrEmployeePeer extends BaseHrEmployeePeer
{
	...
	public static function GetActiveEmployeeListAll()
	{
		$c = new Criteria();
		$c->addJoin(self::EMPLOYEE_NO, PayBasicPayPeer::EMPLOYEE_NO);
		$c->add(PayBasicPayPeer::STATUS, 'A');
...
//		$c->add(self::HR_COMPANY_ID, 2);
	...
	public static function GetIDByEmployeeNo($empno) {
		$c = new Criteria();
		$c->add(self::EMPLOYEE_NO, $empno );
		$rs = self::doSelectone($c);
		return $rs? $rs->getId() : '';
...
//		}
	...
	public static function GetEmployeeNameList($co=null, $user = null) {
		$c = new Criteria();
		$c->add(self::DATE_RESIGNED,  self::DATE_RESIGNED . ' =  "" OR ' . self::DATE_RESIGNED . ' IS NULL ', "CUSTOM");
		$c->addAscendingOrderByColumn(self::NAME);
		if ($co)
		{
...
// 		endif;
		...
		//$c->add(self::BIRTH_PLACE, '&& || ', Criteria::CUSTOM);
		//$c->addJoin(PayBasicPayPeer::EMPLOYEE_NO, self::EMPLOYEE_NO);
		//$c->add(PayBasicPayPeer::STATUS, 'A');
		$rs = self::doSelect($c);
		$val[] = '';
	...
	public static function GetEmployeeNumberNameList($co=null, $user = null) {
		$c = new Criteria();
		$c->add(self::DATE_RESIGNED,  self::DATE_RESIGNED . ' =  "" OR ' . self::DATE_RESIGNED . ' IS NULL ', "CUSTOM");
		$c->addAscendingOrderByColumn(self::NAME);
		if ($co)
...
//	}
	...
	public static function GetEmploymentByName($name)
	{
		//        $c = new Criteria();
		//        $c->add(self::EMPLOYEE_NO, $empNo);
		//        $rs = self::doSelectOne($c);
...
//		exit();
	...
	public static function GetTypeOfEmploymentByEmployeeNo($empNo)
	{
		$c = new Criteria();
		$c->add(self::EMPLOYEE_NO, $empNo);
		$rs = self::doSelectOne($c);
	...
	{
		$speriod = HrLib::CreatePeriodCode($sdate);
		$eperiod = HrLib::CreatePeriodCode($edate);
		$isOneMonth = ($speriod == $eperiod? true : false);
// 		var_dump($speriod);
// 		var_dump($eperiod);
// 		var_dump($isOneMonth);
// 		exit();
		...
		$res = array();
		$periodListing = array('20150401-20150430-ALL-MONTHLY', '20150501-20150531-ALL-MONTHLY', '20150601-20150630-ALL-MONTHLY');
//		$periodListing = array('20150501-20150531-ALL-MONTHLY');
// 		var_dump($speriod);
// 		exit();
		foreach( $rs as $r):
			if ($isOneMonth):
				if (PayEmployeeLedgerArchivePeer::HasDataBankCashNotCash($r->getEmployeeNo(), array($speriod) )):
				...
				if (PayEmployeeLedgerArchivePeer::HasDataBankCashNotCash($r->getEmployeeNo(), $periodListing )):
					if (strtoupper(substr($r->getEmployeeNo(), 0, 1)) <> 'S') :
						$proForiegnNew = PayEmployeeLedgerArchivePeer::isProfessional($r->getProfession()) ? $proForiegnNew + 1 : $proForiegnNew ;
						$cleForiegnNew = PayEmployeeLedgerArchivePeer::isClerk($r->getProfession()) ? $cleForiegnNew + 1 : $cleForiegnNew ;
						$traForiegnNew = PayEmployeeLedgerArchivePeer::isProduction($r->getProfession()) ? $traForiegnNew + 1 : $traForiegnNew ;
...
// 			if ($isOneMonth):
// 				//echo $r->getEmployeeNo() .' '. $r->getName() . ' : ' . $r->getProfession() . '<br>';
// 				$employeeListing[] = $r->getEmployeeNo() .' '. $r->getName() . ' : ' . $r->getProfession() . '<br>';
// 			else:
// 				$employeeListing[] = $r->getEmployeeNo() .' '. $r->getName() . ' : ' . $r->getProfession() . '<br>';
// 			endif;
...
//		echo link_to(image_tag('employee/' . $myphoto,'size="230x300"'),'#', 'id="uploadPhoto" ') ;
	...
	public static function GetPhotoByEmployeeNo($empNo)
	{
//		$myphoto = HrEmployeePeer::GetPhoto($id);
//		echo link_to(image_tag('employee/' . $myphoto,'size="230x300"'),'#', 'id="uploadPhoto" ') ;
		$c = new Criteria();
	...
	public static function GetEmployeeNoWithCPFOneYearAndAboveNoParttime($sdate)
	{
		$year = DateUtils::DUFormat('Y', $sdate) - 1;
		$lastYear = $year . DateUtils::DUFormat('-m-d', $sdate);
		$c = new Criteria();
		$c->addJoin(self::EMPLOYEE_NO, PayBasicPayPeer::EMPLOYEE_NO);
		//$c->add(PayBasicPayPeer::STATUS, 'A');
		$c->add(PayBasicPayPeer::CPF, 'YES');
		$c->add(HrEmployeePeer::COMMENCE_DATE, $lastYear, Criteria::LESS_EQUAL);
		$c->add(HrEmployeePeer::TYPE_OF_EMPLOYMENT, 'PART-TIME', Criteria::NOT_EQUAL);
		$c->addAscendingOrderByColumn(self::NAME);
		$rs = self::doSelect($c);
		$list = array();
		foreach($rs as $r)
	...
	public static function GetEmployeeListByRankCode($empList, $passType)
	{
		$c = new Criteria();
		$c->add(self::NAME, $empList, Criteria::IN);
		if ($passType == 'EPASS' || $passType == 'SPASS'):
			$c->add(self::RANK_CODE, $passType );
		endif;
		if ($passType == 'WP'):
			$c->add(self::RANK_CODE, array('EPASS', 'SPASS'), Criteria::NOT_IN );
		endif;
		$rs = self::doSelect($c);
		$list = array();	
		foreach($rs as $r):
	...
	public static function IsResigned($empno)
	{
		$c = new Criteria();
		$c->add(self::EMPLOYEE_NO, $empno);
		$rs = self::doSelectOne($c);
