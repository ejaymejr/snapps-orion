...
//				                echo $rec->getName().' - '.$rec->getTransDate().': '.$rec->getLevy().' =  '.$levy.'<br>';
				...
				$acDura   = $acDura + ($rec->getAcDura()? $rec->getAcDura() : 0);
				$dailyshift = $rec->getNormal();
//				var_dump($acDura);
//				var_dump($rec->getNormal());
//				var_dump($rec->getName());
//				exit();
...
//				exit();
		...
		$eSum  = array( 'ot1'=>$ot1, 'ot2'=>$ot2, 'ot3'=>$ot3,
						...
						,'acDura' => $acDura, 'shift' => $dailyshift
		);
//		if ($empno == 'S8360501D' )
//			var_dump($eSum);
		return $eSum;
...
//				                echo $rec->getName().' - '.$rec->getTransDate().': '.$rec->getLevy().' =  '.$levy.'<br>';
	...
	public static function GetTotalHoursWorkedPerEmployeeByDate($empno, $sdt, $edt)
	{
		$c = new Criteria();
		$c->add(self::EMPLOYEE_NO, $empno);
		$c->add(self::TRANS_DATE, self::TRANS_DATE .' >= date( "' .$sdt .'" ) && ' . self::TRANS_DATE .' <= date ( "' .$edt . '" )', Criteria::CUSTOM );
		//$c->add(self::ID, '&& || &&', Criteria::CUSTOM);
		$rs = self::doSelect($c);
		$overtimeHours=array('total_overtime' => 0, 'total_normal' => 0, 'normal' => array('total' => 0, 'names' => array() ), 'off_day' => array('total' => 0, 'names' => array() ), 'holiday' => array('total' => 0, 'names' => array() ) );
		foreach ($rs as $r):
			$overtimeHours['total_overtime'] += $r->getOvertimes();
			$overtimeHours['total_normal'] += $r->getNormal();
			switch($r->getMultiplier()):
				case 2.0:
					$overtimeHours['off_day']['total'] += $r->getOvertimes(); 
					//$overtimeHours['off_day']['names'][$r->getTransDate()][] = $r->getName();
					break;
				case 2.5:
					$overtimeHours['holiday']['total'] += $r->getOvertimes();
					//$overtimeHours['holiday']['names'][$r->getTransDate()][] = $r->getName();
					break;
				default:
					$overtimeHours['normal']['total'] += $r->getOvertimes();
					break;
			endswitch;
		endforeach;
// 		echo '<pre>';
// 		print_r($overtimeHours);
// 		echo '</pre>';
// 		exit();
