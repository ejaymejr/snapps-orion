...
class DateUtils
{
    ...
    public static function DUFormat($format, $datetime)
    {
        ...
        $ndate = date($format, mktime($tmp2[0], $tmp2[1], $tmp2[2], $tmp1[1], $tmp1[2], $tmp1[0]));
//        var_dump($tmp2);
//        exit();
        return $ndate;
    }
...
//        exit(); 
    ...
    public static function GetFirstDayOfTheWeek($YrWeekNo)
    {
    	return DateUtils::AddDate(date('Y-m-d',strtotime($YrWeekNo)), -1);
    }
    
    public static function GetLastDayOfTheWeek($YrWeekNo)
    {
    	return DateUtils::AddDate(date('Y-m-d',strtotime($YrWeekNo)), 5);
    }
    
    public static function PeriodToDate($period)
    {
        $date = substr($period, 0, 4) .'-'. substr($period, 4, 2) .'-'. substr($period, 6, 2) .' 00:00:00';
        return $date;
    }
    
    

	public static function GetStartAndEndDateOftheWeekNumber($week, $year)
	{
		$week = $week + 1;
		$day = array('start' => '', 'end'=> '');
        ...
        //var_dump($week);
        //var_dump($week_start->format('Y-m-d'));
        //exit();
        $day['start'] = self::AddDate($week_start->format('Y-m-d'), 1);
        $day['end'] = self::AddDate($day['start'], 6);
        return $day;
	}
