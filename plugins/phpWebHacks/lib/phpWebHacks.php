...
{ 
	...
	protected function fetch($url, $method, $maxredir = 10, $form = array(), $files = array())
	{
		...
		if (array_key_exists('Transfer-Encoding', $head) && $head['Transfer-Encoding'] == 'chunked') {
		...
		if (array_key_exists('Content-Encoding', $head) && $head['Content-Encoding'] == 'gzip') {
	...
	{ 
		foreach($set_cookies as $str) 
		{
			...
			if (isset($secure)) $this->_cookies[$id]['secure'] = $secure;
            if (isset($expires)) $this->_cookies[$id]['expires'] = $expires;
            if (isset($httponly)) $this->_cookies[$id]['httponly'] = $httponly;
	...
	{ 
        ...
        preg_match('/<form[^>]+action\s*=\s*(?(?=[\'"])[\'"]([^\'"]+)[\'"]|\b([^>]+)\b)[^>]*>/siU', $form[1], $a);
