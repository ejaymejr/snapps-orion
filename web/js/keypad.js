	...
	_generateHTML: function(inst) {
		...
		for (var i = 0; i < layout.length; i++) {
			...
			for (var j = 0; j < keys.length; j++) {
				...
				if (keyDef) {
					if ( (inst.options[keyDef.name + 'Text'] || '&nbsp;') == 'Close' )
					{
						html += (keyDef.action ? '<button onclick="AddQuantity()" type="button" class="big ' + this._specialClass +
						...
						html += (keyDef.action ? '<button type="button" class="big ' + this._specialClass +
							' ' + this._namePrefixClass + keyDef.name +
							(inst.options.useThemeRoller ? ' ui-corner-all ui-state-default' +
							(keyDef.noHighlight ? '' : ' ui-state-highlight') : '') +
							'" title="' + inst.options[keyDef.name + 'Status'] + '">' +
					...
					html += '<button type="button" class="big ' + this._keyClass + '"><i class="icon-tag fg-color-green"></i>' +
						(keys[j] == ' ' ? '&nbsp;' : keys[j]) + '</button>';
				}
			}
			//html += '<button type="button" class="big ' + this._keyClass + '"><i class="icon-plus fg-color-green"></i></button>';
			html += '</div>';
		}
		html = $(html);
		var thisInst = inst;
